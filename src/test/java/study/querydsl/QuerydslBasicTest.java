...
public class QuerydslBasicTest {
    ...
    public void searchAndParam() {
        List<Member> result1 = queryFactory
                .selectFrom(member)
                .where(
                        member.username.eq("member1"),  //null 일 경우 해당 where절은 무시 => 동적쿼리에 유용
                        member.age.eq(10)
                )
                .fetch();
...
//        results.getTotal();
     ...
     * SQL: SELECT m.*, t.* FROM Member m LEFT JOIN Team t ON m.TEAM_ID=t.id and
     ...
     내부join일 경우 , join + where 절로도 충분히 해결 가능.
     */
    @Test
    ...
    public void subQuery() throws Exception {
        QMember memberSub = new QMember("memberSub"); //alias
        List<Member> result = queryFactory
                .selectFrom(member)
                .where(member.age.eq(
                        JPAExpressions
    ...
    /** 조건문이 null 일 경우 해당 조건문은 pass 하고
     * 존재하는 조건문만 반영하여 처리한다.*/
    @Test
